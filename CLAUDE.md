...
- **呈現層：** 將日記（實驗組）與傳統儀表板（對照組）呈現給使用者，供 HCI 評估。
...
| 呈現 | Web（FastAPI + 簡易前端）或電子紙 | — |
| 評估 | Python | `scipy`/`pandas`（統計）、問卷 |
...
│   └── server.py              # 儀表板 vs 日記
├── experiments/               # 評估 ─ 角色 E
│   ├── power_log/             # INA221 功耗紀錄
│   ├── calibration/           # 砝重/含水量校準
│   └── hci/                   # 問卷與統計
├── tools/
...
- **DoD（完整）：** 三感測皆實測、每 4h 自動上傳、萬用電表量得睡眠電流 µA 級。
...
### P5 — 實驗與評估 (角色 E，跨期)
1. **技術基準：** INA221 連續記錄功耗（目標 ≥120 小時）；用 5 種含水量砝重做感測校準，算 CV 與 RMSE。
2. **HCI 研究：** 招募約 30 人（22–45 歲）、為期 30 天、A/B 兩組（儀表板 vs 日記）問卷。
- **DoD：** 產出功耗續航數據、校準誤差表、HCI 統計結果與報告。
...
- **成本：** 30 盆 × 30 天 × 每日多次呼叫 LLM 會累積 API 成本，評估期要先估算（詳見 FEASIBILITY）。
