 ...
 **已定案決策（截至 2026-06）：**
 1. **目前範圍＝MVP**：實體感測只接 GPIO34 **土壤濕度**一顆；溫濕度、光照先不接實體感測器。
...
### P1 — 邊緣感測層 (角色 A)
**MVP（現階段）：** 只接 GPIO34 土壤濕度；`temp_c`/`humidity_pct` 由韌體填**模擬值**（或改用 [tools/mock_publisher.py](tools/mock_publisher.py) 產生整包模擬資料），維持 schema 完整；**先不做深度睡眠**（always-on 較好除錯）。待 BME280 到貨再接 I2C，把模擬值換成實測。

1. 接線並對齊腳位（MVP：GPIO34 土壤；〔擴充〕GPIO35 光照、GPIO21/22 I2C 溫濕度）。
...
- **ESP32 ADC 非線性：** GPIO34/35 的 ADC 有已知非線性與雜訊，校準與中位數濾波缺一不可。
