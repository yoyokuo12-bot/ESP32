...
文件版本：v0.2（2026-06，鎖定 MVP 範圍與 BME280；溫濕度暫以模擬資料替代）　目標展示：下週二
...
| 完整目標 | 三感測器、深度睡眠省電、A/B 呈現（儀表板 vs 日記）、功耗與校準實驗 |
| 加分 | HCI 30 人 / 30 天問卷、多盆栽、不同人格 |
| 不在範圍 | 自動澆水/致動器（本專題刻意保留 Human-in-the-loop） |

**已定案（2026-06）：** ① 目前範圍＝MVP，實體感測**僅土壤濕度（GPIO32）**。② 溫濕度感測器選定 **BME280**。
...
| 光照 | LDR + 分壓電路 | 類比輸出 |
| 電源（一般） | **USB 供電即可**（開發 / 展示 / 測試） | 最省事；功耗實驗也能用 USB 串 INA219 量測 |
| 電源（可攜，選用） | TP4056 + 18650 鋰電池 2500 mAh + 電池座 | 電池 3.0–4.2V 未穩壓，**不可直接接 3V3**；需升壓 / buck-boost，或用 18650 shield |
| 量測（功耗實驗） | INA219 / INA226 電壓電流監測模組 | **串接**在供電線量電流；簡單功耗量測有這顆就夠，**不需 18650 shield**（簡報寫 INA221，市售常見 219/226） |

**現階段採購（MVP）：** 只需「電容式土壤濕度感測器 v2.0 + 麵包板 + 杜邦線 + USB 線」。BME280、LDR、TP4056、18650、INA21x 待 MVP 跑通＋到貨再購。

**供電 vs 量測（重要）：** 平常**供電用 USB** 即可；**簡單功耗實驗只需 INA219/226 串接量測**。18650 電池子系統屬「可攜 / 續航」選用，且其升壓器靜態電流會干擾 µA 級量測（見 [FEASIBILITY.md](FEASIBILITY.md) 難點 1）。
...
| GPIO 21 / 22 | I2C (SDA/SCL) | BME280 |
 ...
 MVP 只使用 **GPIO 32（土壤濕度）**；GPIO33（光照）與 GPIO21/22（BME280）待擴充時啟用。

---

## 4. 模組規格與分工 (Modules & Roles)

角色可依實際人數合併。建議 **4–5 人各認領一角**；2–3 人時 A+B、C+D 合併，E 由全員兼任。

### 角色 A — 韌體 / 邊緣感測 (`firmware/`)
- **負責：** ESP32 感測、濾波、MQTT 上傳、（擴充）深度睡眠。
- **MVP 需求（現階段）：**
  - F-A1：讀取 **GPIO32 土壤濕度**；10 次取樣取**中位數**。
...
- **擴充需求（硬體到貨後）：**
  - F-A5：接 BME280（I2C，GPIO21/22）與 LDR（GPIO33），對應欄位由模擬改實測、移除 `sim` 旗標。
...
- **驗收（完整）：** 三感測皆實測、每 4h 上傳、睡眠電流 µA 級。
...
- **需求：**
  ...
  - F-D2：**對照組畫面**＝傳統數據儀表板（簡報 §3 樣式）。
  - F-D3：**實驗組畫面**＝幽默日記時間軸。
  - F-D4：可切換兩組，供 HCI 受試者使用。
- **驗收：** 一次真實喚醒，數分鐘內可見對應的新日記；兩種畫面皆可展示。
- **交付：** `app/server.py`、前端頁面、demo 腳本。

### 角色 E — 研究 / 評估 (`experiments/`)
- **負責：** 技術基準與 HCI 實驗（簡報 §12–§14）。
- **需求：**
  - F-E1 功耗：INA221 連續記錄 ≥120 小時，產出對照組（常時 Wi-Fi）vs 實驗組（深度睡眠）續航。
  - F-E2 校準：5 種含水量砝重（壤質粉砂土 Loamy Silt）烘乾驗證，計算 CV 與 RMSE。
  - F-E3 HCI：招募 ~30 人（22–45 歲）、30 天、A/B 兩組問卷（信任感/同理/照顧行為）。
- **驗收：** 產出功耗表、校準誤差表、HCI 統計與結論。
- **交付：** `power_log/`、`calibration/`、`hci/`（問卷、原始資料、分析腳本）。
...
| 編號 | 需求 | 量化目標（參考簡報結果） |
|---|---|---|
| NFR-1 功耗 | 深度睡眠續航 | 對照組常時 Wi-Fi ~115 mA → ~21.7 小時；實驗組（休眠 ~15 µA、喚醒 ~210 mA、每 4h 喚醒 3–5 秒）→ 目標數月（簡報稱 ~14.5 個月，需實測） |
| NFR-2 數據穩定 | 感測變異係數 (CV) | 6.5%–10.3%；突波濾除率 100% |
| NFR-3 延遲 | 喚醒→日記 | < 數分鐘 |
| NFR-4 內容品質 | 幻覺率 | 抽樣人工檢視，捏造數據/事件 = 0 |
| NFR-5 成本 | LLM API | 評估期先估算每月呼叫量與費用上限 |
